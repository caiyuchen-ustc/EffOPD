...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if data.meta_info.get("enable_iterative_test", False):
                ...
                if test_data is not None and (global_step & (global_step - 1)) == 0:
                    test_results = self._iterative_test_and_modify(
                        ...
                        enable_continue=data.meta_info.get("enable_continue", False)
                    )
                    # 标量 metrics 写入 wandb
                    ...
                    if self.rank == 0:
                        import json
                        print(
                            f"[Iterative Test] history:\n"
                            f"{json.dumps(test_results['history'], indent=2)}"
                        )
    ...
    def _update_base_params(self, backup):
        restore_count = 0
        skip_count = 0
        missing_count = 0
        shape_mismatch_count = 0
        ...
        with torch.no_grad():
            with FSDP.summon_full_params(
                ...
                for name, param in self.base_module_fsdp.named_parameters():
                    if param is None:
                        skip_count += 1
                        continue
        ...
        if self.rank == 0:
            print(
                f"[Iterative Test] Update {restore_count} base parameters "
                f"({total_elements:,} elements, ~{total_bytes / 1024**3:.2f} GB), "
                f"skipped={skip_count}, "
                f"missing={missing_count}, "
                f"shape_mismatch={shape_mismatch_count}"
    ...
    def _iterative_test_and_modify(
        ...
        self._update_base_params(best_backup)
